...
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} "docker run --log-driver=awslogs --log-opt awslogs-region=us-east-2 --log-opt awslogs-group=CompaniesServiceSwagger -d --restart always -e 'API_URL=definitions/swagger.yml' --name companies-swagger --network companies-service-network --ip 172.21.0.3 $REGISTRY_REPO/$SWAGGER:$TAG"
ssh -o StrictHostKeyChecking=no ubuntu@${PRODUCTION_SERVER} "docker run --log-driver=awslogs --log-opt awslogs-region=us-east-2 --log-opt awslogs-group=CompaniesService -d --restart always -e 'FLASK_ENV=production' -e 'FLASK_APP=manage.py' -e 'APP_SETTINGS=project.config.ProductionConfig' -e 'DATABASE_URL=postgres://$DATABASE_PROD_USER:$DATABASE_PROD_PASSWORD@$DATABASE_PROD_URL:$DATABASE_PROD_PORT/$DATABASE_PROD_NAME' -e 'SECRET_KEY=$PROD_SECRET_KEY' -e USERS_SERVICE_URL=$PROD_USERS_SERVICE_URL -e AUTH_SERVICE_URL=$PROD_AUTH_SERVICE_URL --name companies --network companies-service-network --ip 172.21.0.2 $REGISTRY_REPO/$COMPANIES:$TAG"
